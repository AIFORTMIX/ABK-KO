...
If you only need to compile a custom `.c` file into a standalone loadable `.ko`, use the [`build-ko-module.yml`](.github/workflows/build-ko-module.yml) workflow. It can build one Android/KMI target or read the version data under [`data/`](data/) to batch-build the same source for multiple kernel versions. See [Custom `.c` to `.ko` module builds](docs/custom-ko-module.md) for details.
