...
object KernelSupport {
    val buildTargets = listOf(BUILD_TARGET_GKI, BUILD_TARGET_ONEPLUS, BUILD_TARGET_KO)
    ...
    fun normalize(config: KernelBuildConfig): KernelBuildConfig {
        ...
        val isKo = target == BUILD_TARGET_KO
        ...
        val normalizedKsuBranch = normalizeKsuBranch(
            if (isOnePlus || isKo || ksuVariant == KSU_VARIANT_NONE) KSU_BRANCH_STABLE else config.kernelsuBranch
        )
        ...
        return config.copy(
            ...
            kernelsuVariant = if (isKo) KSU_VARIANT_NONE else ksuVariant,
            kernelsuBranch = normalizedKsuBranch,
            customRef = if (isOnePlus || isKo) "" else config.customRef.trim(),
            version = if (isOnePlus || isKo) "" else config.version,
            buildTime = if (isOnePlus || isKo) "" else config.buildTime,
            useZram = if (isOnePlus || isKo) false else config.useZram,
            useDdk = if (isOnePlus || isKo) false else config.useDdk,
            useNtsync = if (isOnePlus || isKo) false else config.useNtsync,
            useNetworking = if (isOnePlus || isKo) false else config.useNetworking,
            useRekernel = if (isOnePlus || isKo) false else config.useRekernel,
            useKpm = when {
                isKo || ksuVariant == KSU_VARIANT_NONE -> false
                isOnePlus -> onePlusKpmSupported && config.useKpm
                else -> gkiKpmSupported && config.useKpm
            },
            cancelSusfs = when {
                isKo || ksuVariant == KSU_VARIANT_NONE -> true
                isOnePlus && !onePlusSusfsEnabled -> true
                else -> config.cancelSusfs
            },
            kpmPassword = if (isOnePlus || isKo || ksuVariant == KSU_VARIANT_NONE || !gkiKpmSupported) "" else config.kpmPassword,
            virtualizationSupport = if (isOnePlus || isKo) "off" else normalizeVirtualizationSupport(line.kernelVersion, config.virtualizationSupport),
            customKernelOptions = if (isOnePlus || isKo) {
                emptyList()
            } else {
                normalizeCustomKernelOptions(config.customKernelOptions)
            },
            useCustomExternalModules = if (isOnePlus || isKo) false else config.useCustomExternalModules,
            customExternalModules = if (isOnePlus || isKo) {
                emptyList()
            } else {
            ...
            koSourcePath = config.koSourcePath.trim().ifBlank { "modules/custom/abk_hello.c" },
            koModuleName = normalizeKoModuleName(config.koModuleName),
            koKernelLocalversion = config.koKernelLocalversion.trim(),
            koExtraCflags = config.koExtraCflags.trim(),
            koBazelLto = if (config.koBazelLto == "none") "none" else "thin"
        )
    }
    ...
    fun normalizeKoModuleName(value: String?): String {
        val normalized = value.orEmpty().trim().replace(Regex("""[^A-Za-z0-9_]"""), "_")
        return normalized.takeIf { it.matches(Regex("""[A-Za-z_][A-Za-z0-9_]*""")) } ?: "abk_hello"
    }
