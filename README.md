...
如果只需要把自定义 `.c` 编译为可单独加载的 `.ko`，可使用 [`build-ko-module.yml`](.github/workflows/build-ko-module.yml) 工作流。它支持单个 Android/KMI 版本构建，也支持读取 [`data/`](data/) 中的版本配置批量为同一份源码生成多个目标内核版本的 `.ko`，详见 [自定义 `.c` 编译为 `.ko` 模块](docs/custom-ko-module.md)。
